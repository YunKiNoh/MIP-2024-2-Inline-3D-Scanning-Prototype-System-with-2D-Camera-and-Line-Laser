...
본 프로젝트는 Line Triangulation의 방법 중에 한 방식인, '카메라를 지상에 수직으로 고정시키고 레이저를 일정 각도로 기울여서 3D 스캐닝을 수행'하는 방식을 채택하였습니다. 다만, 이 방식의 근본적인 문제점은 스캐닝 되는 물체 표면의 각도가 변할 때마다 일정한 시간단위로 조사되고 있는 레이저 사이의 단위 거리가 변한다는 사실입니다. 다만 이러한 오차는 물체의 높이가 낮으면 낮을 수록 줄어들기 때문에 작은 물체에 대한 스캐닝을 진행하는 해당 프로젝트에서는 어느정도 감수가 될지 모르나, 이론적으로 결국 길이 오차가 발생할 수 밖에 없기 때문에 해결할 필요가 있습니다. 이를 해결하기 위해서는 오른쪽 그림과 같이 레이저를 지상에 수직으로 고정하고 카메라는 일정 각도로 기울여서, 물체의 높이에 따른 레이저의 변위를 측정해야 하는 방식을 채택해야 합니다. 이러한 방식 또한 Line Triangulation을 통한 3D 스캐닝 방식 중에 하나로써, 이 방식을 추후에 적용한다면 본 프로젝트의 길이 왜곡에 대한 근본적인 문제점을 해결할 수 있을 것입니다.
