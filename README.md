...
본 프로젝트에서는 3D 이미지의 길이,너비,높이의 단위를 모두 0.1[mm]로 설정하였습니다. 특히 3D 스캐닝의 길이는 컨베이어 벨트와 카메라의 fps가 고려되어야 하는데, 컨베이어 벨트의 속도는 대략 7.5[mm/s]로 측정됩니다. 그리고 이 속도에 대하여 매 프레임마다 캡쳐되는 2D Image 사이의 거리를 0.1[mm]로 맞추기 위해, fps 값은 75로 설정하였습니다. 다만, 이 컨베이어 벨트는 그 속도가 조금씩 변동하고, 이에 따라 매 프레임마다 캡쳐되는 2D Image 사이의 실제 거리는 0.1[mm]로부터 멀어지게 됩니다. 이로 인해 매번 2D Image가 쌓일 때 마다 0.1[mm]의 단위 거리에 대하여 오차가 발생하게 되는데, 이러한 오차가 누적되면 3D 스캐닝의 길이 전체에 왜곡이 발생하게 됩니다. 이를 해결하기 위해서 속도가 안정적으로 유지되며, 추가적으로 미세하게 조절할 수 있는 컨베이어 벨트를 사용한다면, 해당 문제로 인해 발생하는 오차 문제를 개선할 수 있을 것입니다.

### 6.2. Slope Distortion
![image](https://github.com/user-attachments/assets/ac937641-01c3-4307-bd97-74e308bf4843)

본 프로젝트는 Line Triangulation의 방법 중에 한 방식인, '카메라를 지상에 수직으로 고정시키고 레이저를 일정 각도로 기울여서 3D 스캐닝을 수행'하는 방식을 채택하였습니다. 다만, 이 방식의 근본적인 문제점은 스캐닝 되는 물체 표면의 각도가 변할 때마다 일정한 시간단위로 조사되고 있는 레이저 사이의 단위 거리가 변한다는 사실입니다. [왼쪽 그림 참고]. 다만 이러한 오차는 물체의 높이가 낮으면 낮을 수록 줄어들기 때문에 작은 물체에 대한 스캐닝을 진행하는 해당 프로젝트에서는 어느정도 감수가 될지 모르나, 이론적으로 결국 길이 오차가 발생할 수 밖에 없기 때문에 해결할 필요가 있습니다. 이를 해결하기 위해서는 레이저를 지상에 수직으로 고정하고 카메라는 일정 각도로 기울여서, 물체의 높이에 따른 레이저의 변위를 측정해야 하는 방식을 채택해야 합니다. [오른쪽 그림 참고]. 이러한 방식 또한 Line Triangulation을 통한 3D 스캐닝 방식 중에 하나로써, 이 방식을 추후에 적용한다면 본 프로젝트의 길이 왜곡에 대한 근본적인 문제점을 해결할 수 있을 것입니다.
