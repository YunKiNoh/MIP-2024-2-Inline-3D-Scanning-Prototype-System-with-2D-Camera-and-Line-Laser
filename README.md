...
본 프로젝트에서는 3D 이미지의 길이,너비,높이의 단위를 모두 0.1[mm]로 설정하였습니다. 특히 3D 스캐닝의 길이는 컨베이어 벨트와 카메라의 fps가 고려되어야 하는데, 컨베이어 벨트의 속도는 대략 7.5[mm/s]로 측정됩니다. 그리고 이 속도를 바탕으로 각각의 매 프레임마다 캡쳐되는 2D Image 사이의 거리를 0.1[mm]로 맞추기 위해서, fps 값은 75로 설정하였습니다. 다만, 이 컨베이어 벨트는 그 속도가 전압의 변동에 따라 조금씩 유동적이며 이에 따라 측정된 컨베이어 벨트 속도 자체도 정확하지 않기 때문에 0.1[mm] 단위에 대한 오차가 발생합니다. 이러한 오차가 누적되어서 3D 스캐닝의 길이 자체도 왜곡될 수 밖에 없기 때문에 문제가 됩니다. 이를 해결하기 위해서는 우선 그 속도가 안정적인 컨베이어 벨트가 사용한다면 오차를 개선할 수 있을 것입니다.
