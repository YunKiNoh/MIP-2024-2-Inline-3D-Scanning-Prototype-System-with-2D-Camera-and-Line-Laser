...
본 프로젝트에서는 3D 이미지의 길이,너비,높이의 단위를 모두 0.1[mm]로 설정하였습니다. 특히 3D 스캐닝의 길이는 컨베이어 벨트와 카메라의 fps가 고려되어야 하는데, 컨베이어 벨트의 속도를 측정하였을 때 대략 7.5[mm/s]로 결정됩니다. 그리고 이 속도를 바탕으로 각각의 매 프레임마다 캡쳐되는 2D Image 사이의 거리를 0.1[mm]로 맞추기 위해서, fps 값은 75로 설정하였습니다. 다만, 이 컨베이어 벨트는 그 속도가 전압의 변동에 따라 조금씩 유동적이며 이에 따라 측정된 컨베이어 벨트 속도 자체도 정확하지 않기 때문에 0.1[mm] 단위에 대한 오차가 발생합니다. 이러한 오차가 누적되어서 3D 스캐닝의 길이 자체도 왜곡될 수 밖에 없기 때문에 문제가 됩니다. 이를 해결하기 위해서는 우선 그 속도가 안정적인 컨베이어 벨트가 사용한다면 오차를 개선할 수 있을 것입니다.

### 6.2. Slope Distortion
![image](https://github.com/user-attachments/assets/ac937641-01c3-4307-bd97-74e308bf4843)

본 프로젝트는 Line Triangulation의 방법 중에 한 방식인, 카메라를 지상에 수직으로 고정시키고 레이저를 일정 각도로 기울이는 3D 스캐닝 방식을 채택하였습니다. 다만, 이 방식의 근본적인 문제점은 물체의 각도가 변할 때마다 조사되는 레이저 사이의 단위 거리가 변한다는 사실입니다. 다만 이러한 오차는 물체의 높이가 낮으면 낮을 수록 줄어들겠지만, 이론적으로 오차가 발생할 수 밖에 없다는 한계가 있기 때문에 그림과 같이 레이저를 지상에 수직으로 고정하고, 카메라를 기울여서 물체의 높이에 따른 변위를 측정해야 할 것입니다. 이러한 방식 또한 Line Triangulation을 통한 3D 스캐닝 방식 중에 하나로써, 추후에 적용하게 된다면 본 프로젝트의 길이 왜곡에 대한 근본적인 문제점이 해결될 것으로 보입니다.
