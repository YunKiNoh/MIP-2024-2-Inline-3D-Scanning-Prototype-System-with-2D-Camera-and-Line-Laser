...
본 프로젝트에서는 3D 이미지의 (x,y,z) 값, 즉 (길이,너비,높이)의 단위 거리를 모두 0.1[mm]로 통일하였습니다. 특히 3D 스캐닝의 길이는 컨베이어 벨트와 카메라의 fps가 고려되어야 하는데, 컨베이어 벨트의 속도는 대략 7.5[mm/s]로 측정됩니다. 그리고 이 속도에 대하여 매 프레임마다 캡쳐되는 2D Image 사이의 거리를 0.1[mm]로 맞추기 위해, fps 값은 75로 설정하였습니다. 다만, 이 컨베이어 벨트는 그 속도가 조금씩 변동하고, 이에 따라 매 프레임마다 캡쳐되는 2D Image 사이의 실제 거리는 0.1[mm]로부터 멀어지게 됩니다. 이로 인해 매번 2D Image가 쌓일 때 마다 0.1[mm]의 단위 거리에 대하여 오차가 발생하게 되는데, 이러한 오차가 누적되면 3D 스캐닝의 길이 전체에 왜곡이 발생하게 됩니다. 이를 해결하기 위해서 속도가 안정적으로 유지되며, 추가적으로 미세하게 조절할 수 있는 컨베이어 벨트를 사용한다면, 해당 문제로 인해 발생하는 오차 문제를 개선할 수 있을 것입니다.
